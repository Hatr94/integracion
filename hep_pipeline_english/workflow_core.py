...
class HEPWorkflow:
    ...
    def _parse_jet_configs_string(self, txt: str):
        ...
        for item in txt.split(","):
            ...
            R = float(r_str.strip())
            if R <= 0:
                raise ValueError(f"R invalido ({R}). Debe ser > 0.")
    ...
    def configure_analysis(self) to bool:
        print("\n[2/4] CONFIGURACION")
        print("-" * 50)
        ...
        if n_in:
            ...
            if self.n_events <= 0:
                raise ValueError("El numero de eventos debe ser > 0.")

        pt_in = input(f"  pT minimo de jets [GeV] (default {self.jet_pt_min}): ").strip()
        if pt_in:
            self.jet_pt_min = float(pt_in)
            if self.jet_pt_min < 0:
                raise ValueError("El pT minimo debe ser >= 0.")
        ...
        print("\n  Configuracion de jets por R (editable)")
        print("    Formato: R:algoritmo,R:algoritmo,...")
        print("    Algoritmos validos: antikt, kt, cambridge")
        ...
        if self.save_jet_figures:
            nevfig_in = input(
                f"  Max. figures por evento per configuration (default {self.max_event_figures_per_cfg}): "
            ).strip()
            if nevfig_in:
    ...
    def _decompress_lhe_if_needed(self):
        if self.lhe_file.endswith(".gz"):
            ...
            if os.path.exists(out_path):
                ...
                print(f"  Descomprimiendo LHE to {out_path}")
    ...
    def _extract_partons_for_matching(self, pythia):
        """
        Extract relevant partons (quarks and gluon) while avoiding duplicates along decay chains
    ...
    def process_with_pythia_fastjet(self):
        ...
        for i_ev in range(self.n_events):
            ...
            if (i_ev + 1) % 200 == 0:
                print(f"    {i_ev + 1}/{self.n_events} intentos leidos...  aceptados: {accepted_counter}")
