...
class HEPWorkflow:
    ...
    def __init__(self):
        ...
        # --------------------------
        # Pileup (solo overlay, SIN mitigación)
        # --------------------------
        self.enable_pileup = True
        # ↑ Activa/desactiva pileup (overlay de eventos minimum-bias).
        self.mu_pileup = 30.0
        # ↑ Media ⟨μ⟩ del número de colisiones pileup por evento (Poisson).
        self.beam_ecm = 13000.0
        # ↑ Energía del haz (GeV) para el generador de minimum-bias (pp). Ajusta si tu LHE no es 13 TeV.
        self.pythia_pileup_seed = self.pythia_seed + 1
        self.npu_mode  = "poisson"
        self.npu_mean  = 30.0
        self.npu_fixed = 20
        self.mu_pileup = self.npu_mean
        self.pu_mitigation     = "none"
        self.puppi_R0          = 0.3
        self.puppi_Rmin        = 0.02
        self.puppi_wcut        = 0.1
        self.puppi_ptcut_base  = 0.1
        self.puppi_ptcut_slope = 0.007
        # ↑ Semilla distinta para el generador de pileup.
    ...
    def configure_analysis(self) -> bool:
        ...
        # --------------------------
        # Configuración de pileup
        # --------------------------
        print("\n  Configuración de pileup")
        print("  -" * 25)

        pu_in = input(f"  ¿Activar pileup overlay? (s/n, default {'s' if self.enable_pileup else 'n'}): ").strip().lower()
        if pu_in in {"s", "si", "sí", "y", "yes"}:
            self.enable_pileup = True
        elif pu_in in {"n", "no"}:
            self.enable_pileup = False
        # ↑ Si el usuario deja vacío, se conserva el default.

        if self.enable_pileup:
            # ↑ Solo pregunta detalles si el pileup está activado.

            print("  Modo de NPU por evento:")
            print("    1) poisson  -> NPU ~ Poisson(mu)  [recomendado, más realista]")
            print("    2) fixed    -> NPU = valor fijo    [útil para debug]")

            mode_in = input("  Elige modo (1/2, default 1=poisson): ").strip()
            if mode_in == "2":
                self.npu_mode = "fixed"
            else:
                self.npu_mode = "poisson"
            # ↑ Default a Poisson si el usuario presiona Enter o elige 1.

            if self.npu_mode == "poisson":
                mu_in = input(f"  Media del Poisson <mu> (default {self.npu_mean}): ").strip()
                if mu_in:
                    self.npu_mean = float(mu_in)
                if self.npu_mean <= 0:
                    raise ValueError("La media del Poisson debe ser > 0.")
                self.mu_pileup = self.npu_mean  # mantiene alias sincronizado
            else:
                fix_in = input(f"  Valor fijo de NPU (default {self.npu_fixed}): ").strip()
                if fix_in:
                    self.npu_fixed = int(fix_in)
                if self.npu_fixed < 0:
                    raise ValueError("El NPU fijo debe ser >= 0.")
                self.mu_pileup = float(self.npu_fixed)  # mantiene alias sincronizado

            ecm_in = input(f"  Energía del haz para min-bias [GeV] (default {self.beam_ecm}): ").strip()
            if ecm_in:
                self.beam_ecm = float(ecm_in)
                if self.beam_ecm <= 0:
                    raise ValueError("La energía del haz debe ser > 0.")
        ...
        if self.enable_pileup:
            if self.npu_mode == "poisson":
                print(f"    Pileup             : Poisson(mu={self.npu_mean})")
            else:
                print(f"    Pileup             : fijo NPU={self.npu_fixed}")
            print(f"    Beam eCM           : {self.beam_ecm} GeV")
        else:
            print(f"    Pileup             : desactivado")
    ...
    def _init_pythia_pileup(self):
        # ↑ Configura e inicializa una instancia de Pythia para generar minimum-bias (pileup).
        p = pythia8.Pythia()

        # Beam setup (pp, misma energía que tu setup principal)
        p.readString("Beams:idA = 2212")
        p.readString("Beams:idB = 2212")
        p.readString(f"Beams:eCM = {self.beam_ecm}")

        # Minimum-bias / SoftQCD
        # ↑ Esto genera eventos "mínimum-bias" que se usan como pileup.
        p.readString("SoftQCD:all = on")

        # Mantén shower + hadronización (por defecto suelen estar on, lo dejamos explícito)
        p.readString("PartonLevel:MPI = on")
        p.readString("PartonLevel:ISR = on")
        p.readString("PartonLevel:FSR = on")
        p.readString("HadronLevel:Hadronize = on")
        ...
        p.readString("Random:setSeed = on")
        p.readString(f"Random:seed = {self.pythia_pileup_seed}")

        p.readString("Print:quiet = on")

        if not p.init():
            raise RuntimeError("Pythia8 (pileup) no pudo inicializarse. Revisa beam_ecm y configuración SoftQCD.")
    ...
    def process_with_pythia_fastjet(self):
        ...
        pythia_pu = None
        if self.enable_pileup:
            print(f"  Inicializando Pythia8 (pileup minimum-bias), mu={self.mu_pileup} ...")
            pythia_pu = self._init_pythia_pileup()
        ...
        for i_ev in range(self.n_events):
            ...
            idx_user = 0
            # ↑ Contador de user_index global para evitar colisiones cuando se hace overlay de pileup.
            ...
            for i in range(pythia.event.size()):
                ...
                particle_map[idx_user] = (pid, is_charged)
                idx_user += 1
                # ↑ Mapa por user_index para recuperar PID/carga desde constituyentes de FastJet.

                        # --------------------------
            # Pileup overlay (sin mitigación)
            # --------------------------
            if self.enable_pileup and pythia_pu is not None:
                if self.npu_mode == "fixed":
                    n_pu = int(self.npu_fixed)
                else:
                    n_pu = int(self.rng.poisson(self.npu_mean))

                for _ in range(n_pu):
                    if not pythia_pu.next():
                        # ↑ Si el generador de pileup se queda sin eventos (raro), seguimos.
                        continue

                    for j in range(pythia_pu.event.size()):
                        pp = pythia_pu.event[j]
                        if not pp.isFinal():
                            continue
                        if abs(pp.id()) in NEUTRINO_IDS_ABS:
                            continue

                        px, py, pz, e = pp.px(), pp.py(), pp.pz(), pp.e()
                        if (
                            not np.isfinite(px) or not np.isfinite(py) or
                            not np.isfinite(pz) or not np.isfinite(e)
                        ):
