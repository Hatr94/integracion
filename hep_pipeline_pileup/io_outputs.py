...
def save_particles_hdf5(workflow, stored_events, run_dir):
    """Guarda point clouds de partículas en HDF5 para FlowSim.

    Genera dos archivos en run_dir/particles/:
      - events_hard.h5    : solo partículas hard scatter (nivel 1)
      - events_piledup.h5 : hard + PU mezclados (nivel 2, target de FlowSim)

    Cada evento se guarda como dataset separado con shape [N_part, 8]:
      col 0: pt
      col 1: eta
      col 2: phi  (envuelto a (-pi, pi])
      col 3: e
      col 4: |pid|  (PDG absoluto)
      col 5: is_charged  (0/1)
      col 6: vertex_id   (0=hard, k>0=k-esima interaccion PU)
      col 7: is_pu       (0=hard, 1=PU)

    Además guarda un dataset de metadatos por evento:
      "mu"              : NPU real
      "hard_proxy_pt"   : proxy de dureza
      "accepted_idx"    : índice del evento aceptado
    """
    try:
        import h5py
    except ImportError:
        print("  [HDF5] h5py no está instalado. Instala con: pip install h5py")
        print("         Saltando guardado de point clouds.")
        return

    import math

    part_dir = os.path.join(run_dir, "particles")
    os.makedirs(part_dir, exist_ok=True)

    path_hard    = os.path.join(part_dir, "events_hard.h5")
    path_piledup = os.path.join(part_dir, "events_piledup.h5")

    def _extract_matrix(particles, particle_map):
        """Convierte lista de partículas a array [N, 8]."""
        rows = []
        for pj, *_ in particles:
            idx  = pj.user_index()
            info = particle_map.get(idx)
            if info is None:
                continue
            pid, is_charged, vertex_id = info
            phi = float(pj.phi())
            # Envolver phi a (-pi, pi]
            while phi > math.pi:
                phi -= 2.0 * math.pi
            while phi <= -math.pi:
                phi += 2.0 * math.pi
            rows.append([
                ...
                float(pj.e()),
                float(abs(pid)),
                float(int(is_charged)),
                float(vertex_id),
                float(1 if vertex_id > 0 else 0),
            ])
        if not rows:
            return np.empty((0, 8), dtype=np.float32)
        return np.array(rows, dtype=np.float32)

    print(f"\n  Guardando point clouds HDF5 ({len(stored_events)} eventos)...")

    with h5py.File(path_hard,    "w") as fh, \
         h5py.File(path_piledup, "w") as fp:

        # Atributos globales
        for fobj in (fh, fp):
            fobj.attrs["process"]   = str(workflow.process_name or "")
            fobj.attrs["beam_ecm"]  = float(workflow.beam_ecm)
            fobj.attrs["columns"]   = "pt,eta,phi,e,pid_abs,is_charged,vertex_id,is_pu"
            fobj.attrs["n_events"]  = len(stored_events)
        ...
        for ev in stored_events:
            acc_idx = int(ev["accepted_event_idx"])
            mu      = int(ev.get("mu", 0))
            key     = f"ev_{acc_idx:06d}"

            # Hard-only
            mat_hard = _extract_matrix(ev["hard_particles"], ev["hard_particle_map"])
            ds_h = fh.create_dataset(key, data=mat_hard, compression="gzip", compression_opts=4)
            ds_h.attrs["mu"]            = mu
            ds_h.attrs["hard_proxy_pt"] = float(ev["hard_proxy_pt"])
            ds_h.attrs["accepted_idx"]  = acc_idx

            # Piledup (hard + PU)
            mat_pu = _extract_matrix(ev["particles"], ev["particle_map"])
            ds_p = fp.create_dataset(key, data=mat_pu, compression="gzip", compression_opts=4)
            ds_p.attrs["mu"]            = mu
            ds_p.attrs["hard_proxy_pt"] = float(ev["hard_proxy_pt"])
            ds_p.attrs["accepted_idx"]  = acc_idx

            mu_list.append(mu)
            n_hard_list.append(mat_hard.shape[0])
            n_piledup_list.append(mat_pu.shape[0])

        # Vectores globales de metadatos
        fh.create_dataset("meta/mu",           data=np.array(mu_list,        dtype=np.int32))
        fh.create_dataset("meta/n_particles",  data=np.array(n_hard_list,    dtype=np.int32))
        fp.create_dataset("meta/mu",           data=np.array(mu_list,        dtype=np.int32))
        fp.create_dataset("meta/n_particles",  data=np.array(n_piledup_list, dtype=np.int32))

    mu_arr = np.array(mu_list)
    print(f"  ✓ events_hard.h5    -> {len(stored_events)} eventos, "
          f"~{np.mean(n_hard_list):.0f} part/ev")
    print(f"  ✓ events_piledup.h5 -> {len(stored_events)} eventos, "
          f"~{np.mean(n_piledup_list):.0f} part/ev")
    if mu_arr.size > 0:
        print(f"     mu: mean={mu_arr.mean():.1f}, std={mu_arr.std():.1f}, "
              f"min={mu_arr.min()}, max={mu_arr.max()}")

    return path_hard, path_piledup
